...
rem
rem  Deliberately thin.  An earlier version searched for the SAM checkpoint here
rem  and grew a line continuation inside a parenthesised block, which cmd runs as
rem  a command -- so anything worth getting right is done by --sam-model auto in
rem  Python, where it is tested.
rem ---------------------------------------------------------------------------
setlocal
cd /d "%~dp0"
...
set "SAMARGS="
set /p USESAM=Use Segment Anything to mask clutter? [y/N] 
if /i "%USESAM%"=="y" set "SAMARGS=--mask sam --sam-model auto"
...
echo. >> "%LOG%"
"%PY%" rectify.py --sam-info --sam-model auto >> "%LOG%" 2>&1
echo. >> "%LOG%"

rem --- the run ----------------------------------------------------------------
"%PY%" rectify.py "%PHOTOS%" -o "%OUT%\corrected" --debug-dir "%OUT%\debug" --json-report "%OUT%\report.json" --log-file "%LOG%" --diagnostics -v %FOCALARG% %SAMARGS%
