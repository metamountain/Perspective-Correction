...
The strip seam itself (``pipeline.analyse(strip=)`` / ``review.set_strip``) is
...
def test_strip_flag_parses_to_a_pair_and_defaults_off():
    p = cli.build_parser()
    a = p.parse_args(["x.jpg", "--strip", "100", "500"])
    assert list(a.strip) == [100.0, 500.0]
    b = p.parse_args(["x.jpg"])
    assert b.strip is None


def test_job_passes_strip_through_to_process():
    seen = {}

    def fake_process(src, dst, settings, debug_dir=None, dry_run=False, strip=None):
        seen["strip"] = strip
        return "result"
    ...
    try:
        ...
        cli.process = real
    assert out == "result"
    assert seen["strip"] == [100.0, 500.0]


def test_job_leaves_strip_none_when_unset():
    seen = {}
